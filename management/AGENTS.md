...
- Google Sheets là source of truth; YAML IN HOA trong `management/` chỉ là export/cache của sheet quản trị.
- `management/` chỉ được chứa file do workflow Google Sheets management tạo hoặc đọc như cache/export/sync metadata.
- Mọi file không do workflow Google Sheets management tạo hoặc đọc đều không được đặt trong `management/`.
- File dự án không thuộc cache sheet và cần giữ lại đặt trong `artifacts/`; tri thức đã tổng hợp đặt trong `knowledge/`; nháp tạm hoặc input thử nghiệm đặt trong `scratch/`.
