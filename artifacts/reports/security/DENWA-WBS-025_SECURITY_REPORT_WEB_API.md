...
*   **検証エビデンス:**
    ...
    ![アカウントロック画面メッセージ](media/EVD_T01_lockout.png)
    
    ![データベース上の失敗カウント管理](media/EVD_T01_login_failure_count_db.png)
    
    ![HTTP 429 でのレートリミット遮断画面](media/EVD_T01_after_ratelimit_429.png)
...
*   **検証エビデンス:**
    ...
    ![ログインフォームへの特殊文字混入テスト](media/EVD_T02_sqli_login_safe_curl.png)
    
    ![バリデーションエラーによる安全な拒否レスポンス](media/EVD_T02_after_login_safe.png)
...
*   **検証エビデンス:**
    ...
    ![正常値でのスキーマ解決検証](media/EVD_T03_sqli_baseline_curl.png)
    
    ![不正な tenantId の送信拒否](media/EVD_T03_sqli_probe_curl.png)
    
    ![旧ログに記録されていた SQL エラー例外](media/EVD_T03_sqli_psql_cloudwatch.png)
    
    ![不正な tenantId を Java レイヤーでバリデーション遮断したレスポンス](media/EVD_T03_after_probe_400.png)
    
    ![CloudWatch Logs 上でデータベースエラーが検出されないことを確認](media/EVD_T03_after_no_psql_cloudwatch.png)
    
    ![正規テナント ID での正常ログイン確認](media/EVD_T03_after_valid_ok.png)
...
*   **検証エビデンス:**
    ...
    ![以前のセキュリティヘッダー未設定スキャン結果](media/EVD_T04_securityheaders_scan.png)
    
    ![ブラウザ開発者ツールでのヘッダー未適用確認](media/EVD_T04_headers_devtools.png)
    
    ![A+ 評価のスキャン結果](media/EVD_T04_after_securityheaders_AB.png)
    
    ![Nginx SPA レスポンスでのセキュリティヘッダー確認](media/EVD_T04_after_headers_spa_curl.png)
    
    ![API バックエンドレスポンスでのセキュリティヘッダー確認](media/EVD_T04_after_headers_api_curl.png)
...
*   **検証エビデンス:**
    ...
    ![以前の v-html を用いた画面コード](media/EVD_T05_code_Login_62.png)
    
    ![レスポンスメッセージが HTML として解釈されていた状態](media/EVD_T05_vhtml_dom_devtools(api).png)
    
    ![DOM 上で HTML が展開されていた状態](media/EVD_T05_vhtml_dom_devtools.png)
    
    ![エスケープされ安全なテキストノードとして描画された状態](media/EVD_T05_after_text_render.png)
...
*   **検証エビデンス:**
    ...
    ![不正なオリジンからの CORS 要求遮断エビデンス](media/EVD_T06_after_cors_evil_blocked.png)
    
    ![許可された正規オリジンからの接続確認](media/EVD_T06_after_cors_valid_ok.png)
...
*   **検証エビデンス:**
    ...
    ![以前の localStorage 上にパスワードが露出していた状態](media/EVD_T07_localstorage_tempLoginInfo_browser.png)
    
    ![パスワード情報が安全に消去された状態のストレージ](media/EVD_T07_after_no_persistent_creds.png)
...
*   **検証エビデンス:**
    ...
    ![偽装ヘッダーを無視し実 IP に基づき疎通許可されたエビデンス](media/EVD_T08_after_xff_ignored.png)
...
*   **検証エビデンス:**
    ...
    ![閉塞エンドポイントアクセス時の 403 応答](media/EVD_T09_after_sipaccounts_403.png)
    
    ![他テナントリソースへの不正要求ブロック確認](media/EVD_T09_after_no_idor_404.png)
    
    ![USER ロールでの正常アクセス疎通確認](media/EVD_T09_after_messages_role.png)
